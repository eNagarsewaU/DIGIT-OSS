  ...
  processResponse(res) {
    ...
    if (res) {
      sessionStorage.setItem(
        "MODULE_LEVEL",
        JSON.stringify(
          get(res, "MdmsRes.dss-dashboard.dashboard-config[0].MODULE_LEVEL", [])
        )
      );
      sessionStorage.setItem(
        "CHART_COLOR_CODE",
        JSON.stringify(
          get(
            res,
            "MdmsRes.dss-dashboard.dashboard-config[0].CHART_COLOR_CODE",
            []
          )
        )
      );
      sessionStorage.setItem(
        "SERVICES",
        JSON.stringify(
          get(res, "MdmsRes.dss-dashboard.dashboard-config[0].SERVICES", [])
        )
      );
