...
export const getLocaleLabels = (key = "", strings = {}) => {
  ...
  return (
    strings[removeSpaceInLocalisationKey(key)] ||
    removeSpaceInLocalisationKey(key)
  );
};
...
export const fetchLocalisationRequest = (language, tenent) => {
  const reqHeaders = {
    "Content-Type": "application/json",
    Accept: "application/json",
  };
  const reqUrl = `${CONFIGS.LOCALISATION_URL}?locale=${language}&tenantId=${tenent}&module=rainmaker-common,rainmaker-dss`;
  const reqBody = {};
